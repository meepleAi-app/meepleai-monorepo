...
function AgentStatusBadge({ status }: { status: AgentStatus; size?: string; showLabel?: boolean }) {
  const colors: Record<string, string> = { active: 'bg-green-100 text-green-700', idle: 'bg-gray-100 text-gray-600', training: 'bg-blue-100 text-blue-700', error: 'bg-red-100 text-red-700' };
  return <span className={`rounded px-1.5 py-0.5 text-xs font-semibold ${colors[status] ?? ''}`}>{status}</span>;
}
type KbStatus = 'indexed' | 'processing' | 'failed' | 'none';
function KbStatusBadge({ status }: { status: KbStatus; size?: string }) {
  const colors: Record<string, string> = { indexed: 'bg-green-100 text-green-700', processing: 'bg-blue-100 text-blue-700', failed: 'bg-red-100 text-red-700', none: 'bg-gray-100 text-gray-600' };
  return <span className={`rounded px-1.5 py-0.5 text-xs font-semibold ${colors[status] ?? ''}`}>{status}</span>;
}
