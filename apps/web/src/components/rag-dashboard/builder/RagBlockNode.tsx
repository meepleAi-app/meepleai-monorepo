...
/**
 * RAG Pipeline Builder - Custom Block Node
 *
 * ReactFlow custom node for rendering RAG blocks with ports and status.
 *
 * @see #3457 - Setup ReactFlow canvas infrastructure
 */
...
export interface RagBlockNodeProps {
  id: string;
  data: RagNodeData;
  selected?: boolean;
}
...
function RagBlockNodeComponent({ id, data, selected }: RagBlockNodeProps) {
  ...
  return (
    <div
      className={cn(
        'relative min-w-[180px] bg-card rounded-lg border-2 shadow-md transition-all',
...
export const RagBlockNode = memo(RagBlockNodeComponent);

export default RagBlockNode;
