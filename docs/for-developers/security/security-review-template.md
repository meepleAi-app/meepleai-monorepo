...
> 🆕 **Required artifacts** (archived in `docs/for-developers/security/sboms/[QUARTER]/`):

- [ ] Backend SBOM (CycloneDX): `dotnet sbom-tool generate -b apps/api -bo apps/api/_sbom`
  → archive as `docs/for-developers/security/sboms/2026-Q[X]/backend-sbom.cdx.json`
- [ ] Frontend SBOM: `pnpm exec @cyclonedx/cyclonedx-npm --output-file fe-sbom.cdx.json`
  → archive as `docs/for-developers/security/sboms/2026-Q[X]/frontend-sbom.cdx.json`
- [ ] Container SBOM (per image): `syft <image> -o cyclonedx-json`
  → archive per image under `docs/for-developers/security/sboms/2026-Q[X]/containers/`
...
- [ ] **Hadolint**: `hadolint apps/*/Dockerfile infra/*Dockerfile`
  → archive output in `docs/for-developers/security/iac-scans/2026-Q[X]/hadolint.txt`
- [ ] **Trivy config-scan**: `trivy config infra/`
  → archive in `docs/for-developers/security/iac-scans/2026-Q[X]/trivy-config.json`
- [ ] **Checkov**: `checkov -d infra/`
  → archive in `docs/for-developers/security/iac-scans/2026-Q[X]/checkov.json`
...
- [ ] ZAP baseline scan run against staging (unauth):
  → archive in `docs/for-developers/security/dast/2026-Q[X]/zap-baseline-unauth.html`
...
- [ ] Schemathesis or RESTler against OpenAPI on staging:
  ...
  ```
  → archive output in `docs/for-developers/security/dast/2026-Q[X]/schemathesis.log`
...
- [ ] **Three Amigos workshop**: Security Champion + 1 dev/tier-1-BC + 1 QA
  ...
  - Notes archived in `docs/for-developers/security/2026-Q[X]-workshop-notes.md`
