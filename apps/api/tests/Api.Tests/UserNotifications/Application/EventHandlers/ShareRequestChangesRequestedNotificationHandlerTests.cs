...
public sealed class ShareRequestChangesRequestedNotificationHandlerTests
{
    ...
    public ShareRequestChangesRequestedNotificationHandlerTests()
    {
        ...
        _sut = new ShareRequestChangesRequestedNotificationHandler(
            ...
            new Mock<ILogger<ShareRequestChangesRequestedNotificationHandler>>().Object);
    }

    [Fact]
    public async Task Handle_DispatchesChangesRequestedNotification()
    {
        ...
        await _sut.Handle(new ShareRequestChangesRequestedEvent(_shareRequestId, _adminId, "Please add more details"), CancellationToken.None);

        _dispatcher.Verify(d => d.DispatchAsync(
            It.Is<NotificationMessage>(m => m.Type == NotificationType.ShareRequestChangesRequested && m.RecipientUserId == _userId),
    ...
    public async Task Handle_WhenShareRequestNotFound_SkipsNotification()
    {
        ...
        await _sut.Handle(new ShareRequestChangesRequestedEvent(_shareRequestId, _adminId, "Test feedback"), CancellationToken.None);
