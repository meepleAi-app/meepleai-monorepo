...
internal class ApiKeyQuotaEnforcementMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, MeepleAiDbContext db, TimeProvider timeProvider)
    {
        ...
        var quota = ParseQuota(apiKey.Metadata, apiKeyIdStr);
        ...
        // Check hourly limit
        if (!await CheckHourlyLimitAsync(context, db, apiKeyId, apiKeyIdStr, quota, now).ConfigureAwait(false))
        {
            return;
        }

        // Check daily limit
        if (!await CheckDailyLimitAsync(context, db, apiKeyId, apiKeyIdStr, quota, now).ConfigureAwait(false))
        {
            return;
        }
    ...
    private ApiKeyQuota? ParseQuota(string? metadataJson, string? apiKeyIdStr)
    {
        if (string.IsNullOrWhiteSpace(metadataJson)) return null;

        try
        {
            var metadata = JsonSerializer.Deserialize<ApiKeyMetadata>(metadataJson);
    ...
    private async Task<bool> CheckHourlyLimitAsync(
        ...
        DateTimeOffset now)
    {
        if (!quota.HourlyLimit.HasValue) return true;
    ...
    private async Task<bool> CheckDailyLimitAsync(
        ...
        DateTimeOffset now)
    {
        if (!quota.DailyLimit.HasValue) return true;
