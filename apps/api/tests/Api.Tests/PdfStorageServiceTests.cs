...
public class PdfStorageServiceTests : IDisposable
{
    ...
    [Fact(Skip = "Background task DbContext sync issues - needs integration test approach")]
    public async Task UploadPdfAsync_TriggersBackgroundTextExtraction()
    {
        // Arrange
        var (service, dbContext) = CreateServiceWithRealExtraction();
        var tenantId = "tenant1";
        var gameId = "game1";
        var userId = "user1";
        await CreateTestGameAsync(dbContext, tenantId, gameId, userId);
        ...
        fileMock.Setup(f => f.CopyToAsync(It.IsAny<Stream>(), It.IsAny<CancellationToken>()))
            .Returns(async (Stream targetStream, CancellationToken ct) =>
            {
                await targetStream.WriteAsync(pdfBytes, 0, pdfBytes.Length, ct);
            });
        ...
        // Background processing is now synchronous, no need to wait
        // Verify status was updated to completed
        var pdfDoc = await dbContext.PdfDocuments.FindAsync(result.Document.Id);
        Assert.NotNull(pdfDoc);
        Assert.Equal("completed", pdfDoc.ProcessingStatus);
    }

    [Fact(Skip = "Background task DbContext sync issues - needs integration test approach")]
    public async Task BackgroundExtraction_UpdatesStatusToCompleted_WhenSuccessful()
    {
        // Arrange
        var (service, dbContext) = CreateServiceWithRealExtraction();
        var tenantId = "tenant1";
        ...
        // Background processing is synchronous in tests, no delay needed

        // Assert
        var pdfDoc = await dbContext.PdfDocuments.FindAsync(result.Document!.Id);
        Assert.NotNull(pdfDoc);
        Assert.Equal("completed", pdfDoc.ProcessingStatus);
        Assert.NotNull(pdfDoc.ExtractedText);
        Assert.True(pdfDoc.ExtractedText.Length > 0);
        Assert.True(pdfDoc.PageCount > 0);
        Assert.True(pdfDoc.CharacterCount > 0);
        Assert.NotNull(pdfDoc.ProcessedAt);
    }

    [Fact(Skip = "Background task DbContext sync issues - needs integration test approach")]
    public async Task BackgroundExtraction_ExtractsCorrectText()
    {
        // Arrange
        var (service, dbContext) = CreateServiceWithRealExtraction();
        var tenantId = "tenant1";
        ...
        var pdfDoc = await dbContext.PdfDocuments.FindAsync(result.Document!.Id);
        Assert.NotNull(pdfDoc);
        Assert.Contains("Test PDF for background processing", pdfDoc.ExtractedText);
    }
    ...
    public async Task BackgroundExtraction_SetsStatusToFailed_WhenExtractionFails()
    {
        ...
        // Mock PdfTextExtractionService to fail
        var extractionLoggerMock = new Mock<ILogger<PdfTextExtractionService>>();
        var textExtractionServiceMock = new Mock<PdfTextExtractionService>(extractionLoggerMock.Object);
        textExtractionServiceMock
        ...
        var pdfDoc = await dbContext.PdfDocuments.FindAsync(result.Document!.Id);
        Assert.NotNull(pdfDoc);
        Assert.Equal("failed", pdfDoc.ProcessingStatus);
        Assert.NotNull(pdfDoc.ProcessingError);
        Assert.Contains("Test error", pdfDoc.ProcessingError);
        Assert.NotNull(pdfDoc.ProcessedAt);
    }

    [Fact(Skip = "Background task DbContext sync issues - needs integration test approach")]
    public async Task BackgroundExtraction_HandlesMultiPagePdf()
    {
        // Arrange
        var (service, dbContext) = CreateServiceWithRealExtraction();
        var tenantId = "tenant1";
        ...
        var fileMock = new Mock<IFormFile>();
        fileMock.Setup(f => f.FileName).Returns("multipage.pdf");
        fileMock.Setup(f => f.ContentType).Returns("application/pdf");
        fileMock.Setup(f => f.Length).Returns(pdfBytes.Length);
        fileMock.Setup(f => f.CopyToAsync(It.IsAny<Stream>(), It.IsAny<CancellationToken>()))
            .Returns(async (Stream targetStream, CancellationToken ct) =>
        ...
        Assert.Contains("Page 1 content", pdfDoc.ExtractedText);
        Assert.Contains("Page 2 content", pdfDoc.ExtractedText);
        Assert.Contains("Page 3 content", pdfDoc.ExtractedText);
    }

    [Fact(Skip = "Background task DbContext sync issues - needs integration test approach")]
    public async Task BackgroundExtraction_HandlesEmptyPdf()
    {
        // Arrange
        var (service, dbContext) = CreateServiceWithRealExtraction();
        var tenantId = "tenant1";
        ...
        Assert.Equal("completed", pdfDoc.ProcessingStatus);
        Assert.NotNull(pdfDoc.ExtractedText);
        Assert.Equal(0, pdfDoc.CharacterCount);
    }

    [Fact(Skip = "Background task DbContext sync issues - needs integration test approach")]
    public async Task BackgroundExtraction_UpdatesProcessedAtTimestamp()
    {
        // Arrange
        var (service, dbContext) = CreateServiceWithRealExtraction();
        var tenantId = "tenant1";
        ...
        Assert.NotNull(pdfDoc);
        Assert.NotNull(pdfDoc.ProcessedAt);
        Assert.True(pdfDoc.ProcessedAt >= beforeUpload);
        Assert.True(pdfDoc.ProcessedAt <= DateTime.UtcNow.AddSeconds(1));
    }
}
