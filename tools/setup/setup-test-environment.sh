...
if [ "$TEST_FAILURES" -gt 0 ] && [ "$SKIP_TESTS" = false ] && [ "$DRY_RUN" = false ]; then
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        echo ""
        echo -e "${CYAN}👤 Initial Admin User:${NC}"
        echo -e "  ${GREEN}Email:${NC} Set via INITIAL_ADMIN_EMAIL env var (default: admin@example.com)"
        echo -e "  ${GREEN}Password:${NC} Set via INITIAL_ADMIN_PASSWORD env var"
        echo -e "  ${YELLOW}Note:${NC} Additional users must be created via registration or admin panel"
        echo ""
        echo -e "${CYAN}📋 Logs:${NC}"
