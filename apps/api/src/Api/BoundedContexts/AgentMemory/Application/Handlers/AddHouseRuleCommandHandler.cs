...
/// <summary>
/// Handles adding a house rule to a game's memory. Creates GameMemory if none exists.
/// </summary>
internal sealed class AddHouseRuleCommandHandler : ICommandHandler<AddHouseRuleCommand>
{
    private readonly IGameMemoryRepository _gameMemoryRepo;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IFeatureFlagService _featureFlags;
    private readonly ILogger<AddHouseRuleCommandHandler> _logger;

    public AddHouseRuleCommandHandler(
        IGameMemoryRepository gameMemoryRepo,
        IUnitOfWork unitOfWork,
        IFeatureFlagService featureFlags,
        ILogger<AddHouseRuleCommandHandler> logger)
    {
    ...
    public async Task Handle(AddHouseRuleCommand command, CancellationToken cancellationToken)
    {
        ...
        if (memory == null)
        {
            ...
            memory.AddHouseRule(command.Description, HouseRuleSource.UserAdded);
            await _gameMemoryRepo.AddAsync(memory, cancellationToken).ConfigureAwait(false);

            _logger.LogInformation(
                "Created new game memory for game {GameId}, owner {OwnerId} with house rule",
        ...
        else
        {
            ...
            _logger.LogInformation(
                "Added house rule to existing game memory for game {GameId}, owner {OwnerId}",
                command.GameId, command.OwnerId);
        }
