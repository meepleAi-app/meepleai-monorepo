#!/usr/bin/env node
/**
 * DS-13b codemod — admin/knowledge-base migration (Tier 3, sub-b).
 */
...
const files = [...new Set(
  audit.violations
    .filter(v => v.file.startsWith('src/components/admin/knowledge-base/'))
    .map(v => v.file)
)];
...
console.log(`\n[ds-13b-codemod] Modified ${total}/${files.length} files.`);
