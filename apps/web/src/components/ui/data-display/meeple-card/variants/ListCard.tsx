...
export function ListCard(props: MeepleCardProps) {
  ...
  return (
    ...
    >
      <div className="h-2 w-2 flex-shrink-0 rounded-full" style={{ background: entityHsl(entity) }} />
      <div className="h-[52px] w-[52px] flex-shrink-0 overflow-hidden rounded-lg">
        {imageUrl ? (
          <img src={imageUrl} alt={title} className="h-full w-full object-cover" loading="lazy" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-xl opacity-50" style={{ background: entityHsl(entity, 0.08) }}>
            {entityIcon[entity]}
          </div>
        )}
      </div>
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        <h3 className="truncate font-[var(--font-quicksand)] text-[0.88rem] font-bold text-[var(--mc-text-primary)]">{title}</h3>
        {subtitle && <p className="truncate text-[0.75rem] text-[var(--mc-text-secondary)]">{subtitle}</p>}
        <div className="flex items-center gap-2">
          {rating !== undefined && <Rating value={rating} max={ratingMax} />}
          {metadata.length > 0 && <MetaChips metadata={metadata} />}
        </div>
      </div>
      {navItems.length > 0 && <div className="flex-shrink-0"><NavFooter items={navItems} size="sm" /></div>}
    </div>
  );
}
