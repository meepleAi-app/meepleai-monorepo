...
/**
 * UserToolkitConfiguratorClient — User private-game toolkit configurator (Issue #4980).
 *
 * Layout (same as Admin Configurator):
 *  ┌─────────────────────────────────────────────────────────┐
 *  │ ← I miei giochi   Toolkit Configurator        [saving…] │
 *  ├──────────────────────┬──────────────────────────────────┤
 ...
 *  └─────────────────────────────────────────────────────────┘
 *
 * User / PrivateGame owner only — no Publish button (auto-save).
...
function ToolkitPreview({ toolkit }: { toolkit: GameToolkitDto | null }) {
  // Private-game toolkits are never published, but the preview should reflect
  // the configured state (overrides + custom tools), not the published state.
  const previewToolkit = toolkit ? { ...toolkit, isPublished: true } : null;
  const { visibleBaseToolIds, customTools } = resolveSessionTools(previewToolkit);
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="flex items-end gap-2 pb-1">
          <Switch
            ...
            id="dice-interactive-user"
          />
          <Label htmlFor="dice-interactive-user" className="text-xs cursor-pointer">
            Interattivo
          </Label>
        </div>
      </div>
...
// ── UserToolkitConfiguratorContent ────────────────────────────────────────────
...
function UserToolkitConfiguratorContent({ privateGameId }: { privateGameId: string }) {
  ...
  const {
    ...
    addCounterTool,
    removeCounterTool,
  } = usePrivateToolkitEditor(privateGameId);
  ...
  if (isAccessDenied) {
    return (
      <div className="container mx-auto p-6 max-w-4xl">
        {header}
        <AccessDeniedPanel onBack={handleBack} />
      </div>
    );
  }
...
// ── UserToolkitConfiguratorClient (exported) ──────────────────────────────────

export function UserToolkitConfiguratorClient({ privateGameId }: Props) {
  return <UserToolkitConfiguratorContent privateGameId={privateGameId} />;
}
