...
public sealed class ProposalMigrationXminConcurrencyTests : IAsyncLifetime
{
    ...
    public async Task Update_AfterConcurrentWrite_ThrowsConcurrencyException()
    {
        ...
        var seenByFirst = await firstContext.Set<ProposalMigrationEntity>().AsTracking().FirstAsync(e => e.Id == id);
        var seenBySecond = await secondContext.Set<ProposalMigrationEntity>().AsTracking().FirstAsync(e => e.Id == id);
