...
describe('usePdfs', () => {
  ...
  describe('Fetching PDFs', () => {
    it('fetches PDFs when gameId is provided', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('http://localhost:8080/api/v1/games/game-1/pdfs', {
        credentials: 'include',
      });
    });
    ...
    it('logs error to console when fetch fails', async () => {
      ...
      await waitFor(() => {
        expect(mockLoggerError).toHaveBeenCalledWith(
          expect.stringContaining('Failed to load PDFs')
        );
      });
    });
  ...
  describe('Game ID Changes', () => {
    ...
    it('does not refetch if gameId changes to same value', async () => {
      ...
      const { rerender } = renderHook(({ gameId }) => usePdfs(gameId), {
        initialProps: { gameId: 'game-1' },
      });
