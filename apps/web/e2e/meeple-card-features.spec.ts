...
test.describe('MeepleCard — Hover Preview', () => {
  ...
  test('hover su card mostra popover con descrizione', async ({ page }) => {
    ...
    if ((await firstCard.count()) > 0) {
      ...
      const popover = page
        .locator('[data-testid="hover-preview"], [role="tooltip"], .popover')
        .first();
  ...
  test('click wishlist aggiunge gioco e mostra feedback', async ({ page }) => {
    ...
    if ((await firstCard.count()) > 0) {
      ...
      const wishlistBtn = page
        .locator('[data-testid="wishlist-btn"], button[aria-label*="wishlist"]')
        .first();
      ...
      if ((await wishlistBtn.count()) > 0) {
        ...
        const feedback = page.locator('text=/wishlist|aggiunto|added/i, [role="status"]').first();
