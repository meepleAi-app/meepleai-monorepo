...
**New data path (not just a branch condition):** the three-way distinction (`well_formed` abort / `T3_grounding_unavailable` hard-abort / ordinary guardrail-fail retain) requires **validator→pipeline outcome propagation that does not exist today**. Today `MechanicOutputValidator` returns `{IsValid, Violations}` (fail-fast) and `MechanicPipelineResult` has no per-section outcome map (`Outcome` enum = `{Succeeded, AbortedCostCap, AbortedLlmFailed, AbortedValidation}`). This design adds: (a) validator returns `RuleOutcomes` incl. rule strings (D1); (b) `RunSectionAsync` surfaces the final-attempt `RuleOutcomes` + inspects rule strings to pick the branch; (c) `MechanicPipelineResult` gains `SectionOutcomes: IReadOnlyDictionary<MechanicSection, IReadOnlyList<MechanicRuleOutcome>>` that survives to `RunAsync`/the executor. This is a real work item in BE-core, not a one-line conditional.
