...
describe('FE-TEST-010: Upload Queue Integration Tests', () => {
  ...
  describe('Error Handling', () => {
    ...
    it('should handle errors during file conversion', async () => {
      ...
      await act(async () => {
        await result.current.addFiles([invalidFile], 'game-123', 'en');
      });
  ...
  describe('Concurrency Control', () => {
    ...
    it('should handle all slots filled scenario', async () => {
      ...
      await act(async () => {
        await result.current.addFiles([extraFile], 'game-123', 'en');
      });
    ...
    it('should resume processing after all uploads complete', async () => {
      ...
      await act(async () => {
        await result.current.addFiles(firstBatch, 'game-123', 'en');
      });
      ...
      await act(async () => {
        await result.current.addFiles(secondBatch, 'game-123', 'en');
      });
  ...
  describe('Callbacks & Observability', () => {
    ...
    it('should update callbacks when options change', async () => {
      ...
      await act(async () => {
        await result.current.addFiles([file2], 'game-123', 'en');
      });
