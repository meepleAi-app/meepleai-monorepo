using Api.BoundedContexts.KnowledgeBase.Application.Commands.PlaygroundTestScenario;
using Api.BoundedContexts.KnowledgeBase.Domain.Repositories;
using Api.Middleware.Exceptions;
using MediatR;
using Microsoft.Extensions.Logging;

namespace Api.BoundedContexts.KnowledgeBase.Application.Handlers.PlaygroundTestScenario;

/// <summary>
/// Handler for UpdatePlaygroundTestScenarioCommand.
/// Issue #4396: PlaygroundTestScenario Entity + CRUD
/// </summary>
internal sealed class UpdatePlaygroundTestScenarioCommandHandler
    : IRequestHandler<UpdatePlaygroundTestScenarioCommand>
{
    private readonly IPlaygroundTestScenarioRepository _repository;
    private readonly ILogger<UpdatePlaygroundTestScenarioCommandHandler> _logger;

    public UpdatePlaygroundTestScenarioCommandHandler(
        IPlaygroundTestScenarioRepository repository,
        ILogger<UpdatePlaygroundTestScenarioCommandHandler> logger)
    {
        _repository = repository ?? throw new ArgumentNullException(nameof(repository));
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    }

    public async Task Handle(
        UpdatePlaygroundTestScenarioCommand request,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(request);

        var scenario = await _repository.GetByIdAsync(request.Id, cancellationToken).ConfigureAwait(false);
        if (scenario is null)
            throw new NotFoundException($"PlaygroundTestScenario with ID '{request.Id}' not found");

        scenario.Update(
            request.Name,
            request.Description,
            request.Category,
            request.Messages,
            request.ExpectedOutcome,
            request.Tags);

        await _repository.UpdateAsync(scenario, cancellationToken).ConfigureAwait(false);

        _logger.LogInformation(
            "Updated PlaygroundTestScenario {ScenarioId} '{Name}'",
            scenario.Id, scenario.Name);
    }
}
