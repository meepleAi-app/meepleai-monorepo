...
| 3 | Medium/UX | 1.x | Logo `MeepleLogo` SVG legacy (meeple body + ai-spark) usato su pagine `(public)` + `(auth)` + `PublicFooter`. Brand identity incoerente vs `AppTopBar` ("M" gradient canonical). Soluzione: creare `BrandMark` riusabile + sostituire 3 usage + refactor `AppTopBar`. ✅ **RISOLTO**: PR #2065 mergiato in main-dev (commit `315ba8e29`), issue #2057 chiusa. Componente `BrandMark` in `apps/web/src/components/ui/brand/`. ⚠️ **Diagnosi extended**: dopo merge+rebuild il browser continuava a vedere il legacy. Root cause: `next start` orfano in PID 55744 da `D:\Repositories\meepleai-monorepo-dev\apps\web\` (lanciato la sera prima 08/06 23:30) occupava `:3000` co-tenant col Docker — il browser si attaccava al node locale invece del container. Fix: `Stop-Process -Id 55744 -Force`. Lesson: prima di debugging rebuild Docker, sempre `Get-NetTCPConnection -LocalPort 3000` per check duplici listener. | Home `/`, login, register, footer | `meeple-logo.tsx`, `UnifiedHeader.tsx:67`, `AuthLayout.tsx:74`, `PublicFooter.tsx:78` |
