...
def _setup_otel():
    ...
    try:
        ...
        resource = Resource.create({
            "service.name": os.getenv("OTEL_SERVICE_NAME", "unstructured-service"),
...
try:
    from opentelemetry import trace
    _tracer = trace.get_tracer("unstructured-service", "1.0.0")
except Exception:
    pass
...
async def extract_pdf(
    ...
    try:
        ...
        # Extract text off the event loop (CPU-bound) with tracing (Issue #5543)
        span_ctx = _tracer.start_as_current_span("unstructured.extract") if _tracer else None
        span = span_ctx.__enter__() if span_ctx else None
        try:
            if span:
                span.set_attribute("unstructured.file_name", file.filename or "document.pdf")
                span.set_attribute("unstructured.file_size_bytes", file_size)
                span.set_attribute("unstructured.strategy", strategy)
                span.set_attribute("unstructured.language", language)
            ...
            if span:
                span.set_attribute("unstructured.page_count", result.page_count)
                span.set_attribute("unstructured.chunk_count", len(result.chunks))
                span.set_attribute("unstructured.quality_score", result.quality_score.total_score)
                span.set_attribute("unstructured.duration_ms", result.extraction_duration_ms)
                span.set_attribute("unstructured.success", True)
        except Exception as extract_err:
            if span:
                span.set_attribute("unstructured.success", False)
