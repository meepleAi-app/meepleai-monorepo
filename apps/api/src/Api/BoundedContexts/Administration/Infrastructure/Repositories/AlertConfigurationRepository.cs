...
internal class AlertConfigurationRepository : IAlertConfigurationRepository
{
    ...
    {
        var entity = await _context.AlertConfigurations.FindAsync(new object[] { id }, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<AlertConfiguration?> GetByKeyAsync(string configKey, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await _context.AlertConfigurations.FirstOrDefaultAsync(c => c.ConfigKey == configKey, cancellationToken).ConfigureAwait(false);
        return entity == null ? null : MapToDomain(entity);
    }
    ...
    public async Task<List<AlertConfiguration>> GetByCategoryAsync(ConfigCategory category, CancellationToken cancellationToken = default) =>
        (await _context.AlertConfigurations.Where(c => c.Category == category.ToDisplayString()).ToListAsync(cancellationToken).ConfigureAwait(false)).Select(MapToDomain).ToList();

    public async Task<List<AlertConfiguration>> GetAllAsync(CancellationToken cancellationToken = default) =>
        (await _context.AlertConfigurations.ToListAsync(cancellationToken).ConfigureAwait(false)).Select(MapToDomain).ToList();

    public async Task AddAsync(AlertConfiguration config, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(config);
        await _context.AlertConfigurations.AddAsync(MapToEntity(config), cancellationToken).ConfigureAwait(false);
    ...
    {
        ArgumentNullException.ThrowIfNull(config);
        var entity = await _context.AlertConfigurations.FindAsync(new object[] { config.Id }, cancellationToken).ConfigureAwait(false);
    ...
    {
        var entity = await _context.AlertConfigurations.FindAsync(new object[] { id }, cancellationToken).ConfigureAwait(false);
        if (entity != null) { _context.AlertConfigurations.Remove(entity); await _context.SaveChangesAsync(cancellationToken).ConfigureAwait(false); }
    }
