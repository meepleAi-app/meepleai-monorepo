#!/usr/bin/env python3
"""
FluentAssertions Migration Script for RuleSpecServiceTests.cs
Converts xUnit Assert.* patterns to FluentAssertions syntax

Phase 5B Target: 92 assertions in RuleSpecServiceTests.cs
Distribution: Equal(61), Contains(7), Single(6), True(2), Collection(?)
"""
...
def main():
    file_path = "tests/Api.Tests/RuleSpecServiceTests.cs"
    ...
    if content != original_content:
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"\n[SUCCESS] Successfully migrated {total_conversions} assertions in RuleSpecServiceTests.cs")
        return 0
    else:
