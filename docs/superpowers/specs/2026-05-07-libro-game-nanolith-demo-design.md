...
| D19 | **Iter 1.A delivered** (2026-05-07, branch `feature/libro-game-iter-1a`, 11 commit pre-PR): G1+G2+G4-base. Nuovo aggregate `GamebookCampaignSession` (SessionTracking BC) + EF migration `AddGamebookCampaignSessions` (jsonb progress, schema `session_tracking`) + 4 endpoint REST `/api/v1/gamebook/campaigns` + frontend route `/library/games/[gameId]/play/[campaignId]` + `GamebookPlayShell` (FREEZE-compliant) + `NewCampaignDialog` con CTA gated su `gameTitle === 'Nanolith'` (proper `hasGamebook` field deferred Iter 2). Tests: 13 backend unit pass (4 VO + 4 aggregate + 5 handler), 13 frontend unit pass (8 client + 2 hook + 3 shell + 2 dialog), 1 E2E `@ci` smoke + 1 `@dogfood` skip placeholder + 1 backend golden eval skip placeholder. **Plan deviations**: (a) Task 10 integration test endpoint SKIPPED for pace — coperto da `@ci` E2E smoke + 13 unit; (b) plan assumeva `SessionTrackingDbContext` ma codebase usa `MeepleAiDbContext` condiviso con `ApplyConfigurationsFromAssembly` (modern direct-domain-mapping pattern come `ToolkitSessionState`); (c) auth helper è `context.TryGetAuthenticatedUser()` + `TryGetUserId()` non `RequireUserSession`; (d) endpoint registration via `RouteGroupBuilder` extension method, NON `IEndpointRouteBuilder`; (e) exception namespace `Api.Middleware.Exceptions`. Subagent-driven workflow: 7 dispatch implementer (sonnet) + 1 dual review Task 1 + 1 spec/quality Task 2. | Plan completed |
| D20 | **Iter 1.B delivered** (2026-05-08, branch `feature/libro-game-iter-1b` based on `feature/libro-game-iter-1a`, ~7 commit pre-PR): G3+G4-full. 3 nuove entità domain (`GamebookPhotoArtifact` state machine, `TranslatedParagraph` history, `GamebookGlossaryEntry`) + EF migration `AddGamebookPhotoAndGlossaryTables` (3 tabelle, jsonb segments, text[] applied terms) + 2 service infra (`IGamebookPhotoStorage` w/ EXIF strip via ImageSharp, `IOcrService` Tesseract impl) + 4 application command/query (Upload, Segment, TranslateGamebookSegmentQuery streaming via `IStreamingQueryHandler`, BootstrapGlossary + GetHistory + GetGlossary + UpsertGlossaryEntry) + 7 endpoint REST/SSE sotto `/api/v1/gamebook/campaigns/{id}/{photos,history,glossary,...}` + frontend route `/play/[campaignId]/translate` + 3 component (`TranslateViewer` simplified-w/-`<input capture>`, `SegmentPicker`, `TranslationPane`) + 5 hook (Photo upload, Segment, SSE translate, Glossary, History). **Test**: ~17 backend unit (10 domain + 4 storage + 7 application — 1 skip Tesseract + 1 skip Bootstrap), ~49 frontend unit (schema + hooks + components), 1 E2E `@ci` smoke + 1 `@dogfood` placeholder. **Plan deviations** (documentate in dispatch logs): (a) `IBlobStorageService` riusato come backing store anziché creare wrapper S3 dedicato; (b) `IStreamingQueryHandler<,>` pattern già esistente — niente CQRS carve-out come temuto dal plan §4.3 D9; (c) Bootstrap glossary usa solo `gameId` come hint LLM (no KB integration in Iter 1.B — Aaron edita post-bootstrap via UpsertGlossary endpoint); (d) FE: `<input type="file" capture="environment">` invece di `CameraViewfinder` (ScopeSimplification: `CameraViewfinder` richiede MediaStream + light meter + page-detection — over-scope per dogfood); (e) Task 11+12 integration test full pipeline + consistency rate SKIP per pace (Aaron-validated); (f) Task 16 GlossaryEditor + Task 17 HistoryDrawer + Task 20 Cleanup job DEFER a Iter 1.C (defer-list pre-approvato dal plan). | Plan completed (critical path) |
