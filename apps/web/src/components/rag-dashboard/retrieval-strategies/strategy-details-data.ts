...
export const STRATEGY_DETAILS: Record<RetrievalStrategyType, StrategyDetailContent> = {
  Hybrid: {
    ...
    flowSteps: [
      { id: 'query', label: 'User Query', description: 'Natural language question', icon: '💬', color: 'blue' },
      { id: 'parallel', label: 'Parallel Search', description: 'Vector + Keyword simultaneously', icon: '⚡', color: 'purple' },
      { id: 'vector', label: 'Vector Search', description: 'Semantic embedding similarity', icon: '🧠', color: 'cyan' },
      { id: 'keyword', label: 'Keyword Search', description: 'BM25 term matching', icon: '🔑', color: 'green' },
      { id: 'fusion', label: 'Result Fusion', description: 'Reciprocal Rank Fusion (RRF)', icon: '🔀', color: 'orange' },
      { id: 'rerank', label: 'Reranking', description: 'Cross-encoder scoring', icon: '📊', color: 'red' },
    ],
    example: {
      query: 'How do I score points in Catan?',
      context: 'Player asking about victory conditions in Settlers of Catan',
      response: 'In Catan, you score Victory Points (VPs) through: settlements (1 VP each), cities (2 VP each), Longest Road (2 VP), Largest Army (2 VP), and certain Development Cards. First to 10 VPs wins!',
      highlight: 'Hybrid search found both semantic matches for "scoring" and exact matches for "Victory Points"',
    },
    technicalNotes: [
  ...
  Semantic: {
    ...
    flowSteps: [
      { id: 'query', label: 'User Query', description: 'Natural language question', icon: '💬', color: 'blue' },
      { id: 'embed', label: 'Query Embedding', description: 'Convert to vector', icon: '🔢', color: 'purple' },
      { id: 'search', label: 'Vector Search', description: 'ANN similarity search', icon: '🧠', color: 'cyan' },
      { id: 'retrieve', label: 'Retrieve Docs', description: 'Top-k similar chunks', icon: '📄', color: 'green' },
      { id: 'return', label: 'Return Results', description: 'Ranked by similarity', icon: '📊', color: 'orange' },
    ],
    example: {
      query: 'What happens when I run out of resources?',
      context: 'Player asking about resource management in various games',
      response: 'When resources are depleted, you typically cannot perform actions requiring them until you acquire more. Some games have scarcity mechanics, trading systems, or resource regeneration phases.',
  ...
  Keyword: {
    ...
    flowSteps: [
      ...
      { id: 'tokenize', label: 'Tokenization', description: 'Extract terms', icon: '✂️', color: 'purple' },
      { id: 'bm25', label: 'BM25 Scoring', description: 'Term frequency analysis', icon: '📊', color: 'green' },
      { id: 'retrieve', label: 'Retrieve Docs', description: 'Ranked by score', icon: '📄', color: 'orange' },
      { id: 'return', label: 'Return Results', description: 'Top matches', icon: '✅', color: 'cyan' },
    ],
    example: {
      query: 'What is the "Robber" in Catan?',
      context: 'Player asking about a specific game component',
      response: 'The Robber is a piece that blocks resource production on a hex. When a 7 is rolled, the active player must move the Robber to any hex (except desert) and steal one resource from an opponent with a settlement or city there.',
  ...
  Contextual: {
    ...
    flowSteps: [
      { id: 'query', label: 'User Query', description: 'Current question', icon: '💬', color: 'blue' },
      { id: 'history', label: 'Load History', description: 'Previous turns', icon: '📜', color: 'purple' },
      { id: 'rewrite', label: 'Query Rewrite', description: 'Contextualize query', icon: '✏️', color: 'cyan' },
      { id: 'search', label: 'Enhanced Search', description: 'With context', icon: '🔍', color: 'green' },
      { id: 'filter', label: 'Context Filter', description: 'Topic relevance', icon: '🎯', color: 'orange' },
      { id: 'return', label: 'Return Results', description: 'Contextual matches', icon: '✅', color: 'red' },
    ],
    example: {
      query: 'What about the blue ones?',
      context: 'Previous question was about Catan resource cards',
      response: 'The blue resource cards in Catan represent Ore. Ore is primarily used for building cities (3 Ore + 2 Wheat) and buying Development Cards (1 Ore + 1 Wheat + 1 Wool).',
      highlight: 'Contextual search understood "blue ones" refers to Ore cards from conversation history',
    },
    technicalNotes: [
  ...
  MultiQuery: {
    ...
    flowSteps: [
      { id: 'query', label: 'User Query', description: 'Original question', icon: '💬', color: 'blue' },
      { id: 'expand', label: 'Query Expansion', description: 'LLM generates variants', icon: '🤖', color: 'purple' },
      { id: 'parallel', label: 'Parallel Search', description: 'All queries simultaneously', icon: '⚡', color: 'cyan' },
      { id: 'collect', label: 'Collect Results', description: 'From all queries', icon: '📥', color: 'green' },
      { id: 'dedup', label: 'Deduplicate', description: 'Remove duplicates', icon: '🔄', color: 'orange' },
      { id: 'rank', label: 'Final Ranking', description: 'Combined relevance', icon: '📊', color: 'red' },
    ],
    example: {
      query: 'How do alliances work in Risk?',
      context: 'Player asking about diplomacy mechanics',
      response: 'In Risk, alliances are informal agreements between players. While there are no official alliance rules, players often form temporary pacts for mutual defense, coordinate attacks, or agree not to attack each other. These alliances can be broken at any time, making diplomacy a key strategic element.',
      highlight: 'Multi-Query expanded to: "alliance rules", "diplomacy in Risk", "player agreements", "breaking alliances"',
    },
    technicalNotes: [
  ...
  Agentic: {
    ...
    flowSteps: [
      { id: 'query', label: 'User Query', description: 'Complex question', icon: '💬', color: 'blue' },
      { id: 'plan', label: 'Plan Steps', description: 'Agent decides approach', icon: '🎯', color: 'purple' },
      { id: 'retrieve', label: 'Retrieval Loop', description: 'Iterative searching', icon: '🔄', color: 'cyan' },
      { id: 'reason', label: 'Reasoning', description: 'Analyze findings', icon: '🧠', color: 'green' },
      { id: 'verify', label: 'Verification', description: 'Check consistency', icon: '✅', color: 'orange' },
      { id: 'synthesize', label: 'Synthesize', description: 'Final answer', icon: '📝', color: 'red' },
    ],
    example: {
      query: 'In a 4-player Catan game, if two players tie for Longest Road but a third player has the actual longest road, who gets the 2 VP?',
      context: 'Complex edge case requiring multi-step reasoning',
      response: 'The player with the actual longest road gets the 2 VP. Longest Road only considers the single longest continuous road each player has, not tied positions. If two players have roads of equal length, neither gets the bonus until one breaks the tie with a longer road. The third player with the longer road would hold the bonus.',
      highlight: 'Agent performed: 1) Search Longest Road rules, 2) Search tie-breaking rules, 3) Verify with official FAQ, 4) Synthesize answer',
    },
    technicalNotes: [
