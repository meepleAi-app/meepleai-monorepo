...
/// <summary>
/// Handler for bulk rejection of share requests.
/// Issue #2893: All-or-nothing transaction - if any rejection fails, entire batch rolls back.
///
/// ARCHITECTURE DECISION: Uses strict all-or-nothing transaction semantics (differs from BulkPasswordReset/BulkRoleChange
/// which allow partial success). Rationale: Share request rejection is a coordinated editorial decision where
/// partial failures could create inconsistent catalog state. Per DoD requirement: "Transaction: All or nothing".
/// </summary>
internal sealed class BulkRejectShareRequestsCommandHandler : ICommandHandler<BulkRejectShareRequestsCommand, BulkOperationResult>
{
    ...
    public async Task<BulkOperationResult> Handle(
        BulkRejectShareRequestsCommand command,
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(
                "Bulk reject operation exceeds limit: {Count} > {MaxLimit}",
        ...
        {
            _logger.LogWarning(
                "Bulk reject request contains {DuplicateCount} duplicate share request IDs",
                command.ShareRequestIds.Count - distinctIds.Count);
        }

        _logger.LogInformation(
            "Editor {EditorId} initiating bulk rejection of {Count} share requests. Reason: {Reason}",
            command.EditorId, distinctIds.Count, command.Reason);
        ...
        try
        {
            ...
            foreach (var shareRequestId in distinctIds)
            {
                ...
                // Reject the request (domain validates and raises events)
                // Will throw if: not in InReview status, lock expired, reviewer mismatch, etc.
                shareRequest.Reject(command.EditorId, command.Reason);

                _shareRequestRepository.Update(shareRequest);

                // Create audit log entry for each rejection
            ...
            _logger.LogInformation(
                "Bulk rejection completed successfully: {Count} share requests rejected by editor {EditorId}",
...
#pragma warning restore S125
        ...
        {
            _logger.LogError(ex,
                "Bulk rejection failed for editor {EditorId} - transaction rolled back. {Count} share requests not rejected.",
