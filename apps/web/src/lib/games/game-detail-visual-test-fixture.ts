 ...
 * is baked into the build, the orchestrator (GameDetailView) substitutes the
 ...
 *
 * Used by:
 *   - `apps/web/e2e/visual-migrated/sp4-game-detail.spec.ts` (Task 4)
 *   - `apps/web/e2e/v2-states/game-detail.spec.ts` (Task 4)
 *
 * Mirror of: `apps/web/src/lib/games/visual-test-fixture.ts` (Wave B.1 pattern)
 */
...
const FIXTURE_DEFAULT: LibraryGameDetail = {
  ...
  isFavorite: true,
  currentState: 'Owned',
  stateChangedAt: '2026-05-03T10:00:00.000Z',
  stateNotes: null,
 ...
 * Returns `null` for `'not-found'` state always. Returns `null` in dev/prod
 * builds — caller MUST fall through to the real fetch.
 *
 * @param state - Which fixture state to load (default: 'default')
 * @returns LibraryGameDetail for 'default' state in visual-test builds, null otherwise
 */
