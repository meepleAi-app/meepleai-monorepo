...
import { buildKbNavItems } from '../buildKbNavItems';

describe('buildKbNavItems', () => {
  const handlers = {
    onChunksClick: vi.fn(),
  ...
  it('returns 4 nav items in canonical order: Chunks, Reindex, Anteprima, Download', () => {
    const items = buildKbNavItems({ chunkCount: 124 }, handlers);
  ...
  it('disables Chunks when chunkCount is undefined', () => {
    const items = buildKbNavItems({}, handlers);
    expect(items[0].disabled).toBe(true);
  });

  it('reindex/preview/download are pure action slots (no count)', () => {
    const items = buildKbNavItems({ chunkCount: 1 }, handlers);
