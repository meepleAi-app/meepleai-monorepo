...
4. **Impatto su #1889**: il blocco *"5 mockup agent-builder mancanti (D–H)"* si **dissolve del tutto**. D–G esistono come admin FE shipped; **anche H è coperto** (verifica 2026-07-15: `/library/[gameId]/agent` è un redirect 307 al tab `aiChat` già shipped — `GameAiChatTab`/`GameChatTab` + `POST /api/v1/agents/qa/stream` — e il mockup `sp4-game-chat-tab.html` copre lo scope; la standalone view fu ritirata di proposito). **Nessun mockup agent-builder da autorare.** Il demo replay #1889, se rieseguito, va scopato **solo su game-night**, non sull'agent-builder.
