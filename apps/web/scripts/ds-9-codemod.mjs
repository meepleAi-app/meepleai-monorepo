#!/usr/bin/env node
/**
 * DS-9 codemod — players + player-detail cluster migration.
 * Mirrors DS-8 logic, scoped to player surfaces.
 */
...
const files = [...new Set(
  audit.violations
    .filter(v =>
      v.file.startsWith('src/components/features/players/') ||
      v.file.startsWith('src/components/features/player-detail/') ||
      v.file.startsWith('src/components/player/') ||
      (v.file.includes('app/(authenticated)/players/'))
    )
    .map(v => v.file)
)];
...
console.log(`\n[ds-9-codemod] Modified ${total}/${files.length} files.`);
