...
**Total Issues:** 38 (26 open, 12 completed: EDIT-04, EDIT-05, EDIT-06, ADMIN-02, AI-13, AI-14, N8N-04, N8N-05, AI-15-ALT, OPS-07, TEST-03/#601, TEST-05/#597)
...
| # | Issue | Priority | Effort | Rationale |
|---|-------|----------|--------|-----------|
| 18 | #461 ADMIN-01: Prompt management | ✅ COMPLETED | XL | **COMPLETE - All 5 Phases Done (100%)**: Phase 1-5 complete. Phase 4: PromptEvaluationService, 5-metric engine, A/B comparison, security hardened, 4 test datasets, 17 unit tests. Phase 5: OpenTelemetry metrics, Prometheus alerts, deployment runbook. Total: 8,100+ LOC across 17 commits (PR #553 + direct commits). PRODUCTION-READY system with monitoring. See `ADMIN-01-PHASE4-COMPLETION-CERTIFICATE.md`. Completed: 2025-10-26 |
...
| 34 | #559 AUTH-06-P4: Security hardening | 🔄 IN PROGRESS | L | **P4 SECURITY**: Email verification before linking + rate limiting on OAuth endpoints. Prevents account takeover and abuse. 20+ tests. Effort: 22-24h (2.5-3 days). |
...
| 37 | #597 TEST-05: BackgroundServiceTestHelper | ✅ COMPLETE | M (8h actual, 8h est) | **100% COMPLETE**: Fixed all 7+ failing background service tests using hybrid approach. **Solution**: Direct Method Testing (primary) + BackgroundServiceTestHelper (lifecycle). Made internal methods testable (WarmCacheAsync, GenerateScheduledReportAsync). **Results**: 15/15 tests passing (CacheWarmingServiceTests 6/6, QualityReportServiceTests 9/9), 0% flakiness (3/3 runs pass), ~1s execution. **Deliverables**: BackgroundServiceTestHelper (~150 LOC), test fixes, comprehensive documentation (~650 lines). Direct method testing eliminates async coordination complexity. PR #605. Branch: `DegrassiAaron/issue597`. Completed: 2025-10-31 |
| 38 | #604 TEST-04: Testcontainers concurrency tests | ✅ COMPLETED | XL (24-32h actual) | **IMPLEMENTED**: 14 Testcontainers-based concurrency tests for 3 critical services (RuleSpec, SessionManagement, PromptTemplate). All tests use ConfigurationConcurrencyTests pattern (WebApplicationFactory + Testcontainers + PostgreSQL). 100% pass rate (14/14). All 4 patterns covered (Lost Update, Optimistic, TOCTOU, Cache). ChatService deferred due to complex API architecture. 1,173 LOC. PR #606. Completed: 2025-10-31 |
