...
/// <summary>
/// Integration tests for UnsuspendUserCommandHandler.
...
public sealed class UnsuspendUserCommandHandlerIntegrationTests : IAsyncLifetime
{
    ...
    private UnsuspendUserCommandHandler _handler = null!;

    public UnsuspendUserCommandHandlerIntegrationTests(SharedTestcontainersFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        _dbName = $"test_unsuspend_user_{Guid.NewGuid():N}";
        ...
        _handler = new UnsuspendUserCommandHandler(_userRepository, _auditLogRepository, _unitOfWork);
    }
    ...
    public async Task Handle_WithValidCommand_PersistsAuditLog()
    {
        ...
        var command = new UnsuspendUserCommand(user.Id.ToString(), requesterId);

        // Act
        var result = await _handler.Handle(command, TestContext.Current.CancellationToken);

        // Assert - verify user unsuspended
        result.IsSuspended.Should().BeFalse();
        result.SuspendReason.Should().BeNull();

        // Assert - verify audit log persisted
        var auditLogs = await _dbContext.AuditLogs
            .Where(a => a.Action == "user_unsuspended" && a.ResourceId == user.Id.ToString())
            .ToListAsync();
    ...
    [Fact]
    public async Task Handle_WithValidCommand_RaisesUserUnsuspendedEvent()
    {
        // Arrange
        var userId = Guid.NewGuid();
        var requesterId = Guid.NewGuid();
        ...
        var unsuspendedUser = await _userRepository.GetByIdAsync(user.Id, CancellationToken.None);
        unsuspendedUser.Should().NotBeNull();
        unsuspendedUser!.IsSuspended.Should().BeFalse();
        unsuspendedUser.SuspendReason.Should().BeNull();
        unsuspendedUser.SuspendedAt.Should().BeNull();
    }
}
