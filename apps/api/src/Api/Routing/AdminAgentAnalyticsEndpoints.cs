...
internal static class AdminAgentAnalyticsEndpoints
{
    public static RouteGroupBuilder MapAdminAgentAnalyticsEndpoints(this RouteGroupBuilder group)
    {
        ...
        // GET /api/v1/admin/agents/models (real data from AgentTestResult)
        agentsGroup.MapGet("/models", async (
            HttpContext context,
            IMediator mediator,
            CancellationToken ct) =>
        {
            ...
            var result = await mediator.Send(new GetAgentModelsQuery(), ct).ConfigureAwait(false);

            return Results.Ok(new { models = result.Models });
        });
