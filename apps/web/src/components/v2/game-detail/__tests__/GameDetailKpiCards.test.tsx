...
describe('GameDetailKpiCards (Wave C.1)', () => {
  ...
  it('renders without unit when unit is undefined', () => {
    ...
    const card = container.querySelector('[data-kpi-key="players"]');
    expect(card).toBeInTheDocument();
    // No unit span rendered
    expect(card?.querySelectorAll('.text-\\[13px\\]')).toHaveLength(0);
  });
  ...
  it('exposes data-slot="game-detail-kpi-cards" for E2E selector', () => {
    const { container } = render(<GameDetailKpiCards cards={cards} />);
    expect(container.querySelector('[data-slot="game-detail-kpi-cards"]')).toBeInTheDocument();
  });
});
