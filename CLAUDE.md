...
**Storybook snapshot scaffolding — DS-17-8-v2 (#2095)** — `playwright.storybook.config.ts` + `e2e/storybook/*.snapshot.spec.ts` Playwright suite per 3 pilot stories (threshold 5% area, 1440x900 desktop, light theme). CI step `Storybook snapshot scaffolding` `continue-on-error: true` (non-blocking). **Baseline PNGs deferred a Phase 4 hardening**: pilot Client components chiamano `useTranslation` ma `.storybook/preview.tsx` non wira `IntlProvider` ancora → stories rendono error wall su useIntl. Phase 4 risolve decorator + capture baseline. Local: `pnpm test:storybook:snapshots[:update]`.
