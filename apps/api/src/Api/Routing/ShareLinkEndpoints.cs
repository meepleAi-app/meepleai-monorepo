...
internal static class ShareLinkEndpoints
{
    public static RouteGroupBuilder MapShareLinkEndpoints(this RouteGroupBuilder group)
    {
        ...
        group.MapPost("/share-links", HandleCreateShareLink)
        .WithName("CreateShareLink")
        .RequireSession() // ISSUE-1446: Automatic session validation
        .WithTags("ShareLinks");

        // 2. Revoke shareable link (authenticated)
        group.MapDelete("/share-links/{id}", HandleRevokeShareLink)
        .WithName("RevokeShareLink")
        .RequireSession() // ISSUE-1446: Automatic session validation
        .WithTags("ShareLinks");

        // 3. Get shared thread (public, no session required)
        group.MapGet("/shared/thread", HandleGetSharedThread)
        ...
        group.MapPost("/shared/thread/comment", HandleAddCommentToSharedThread)
    ...
    private static async Task<IResult> HandleCreateShareLink(
    ...
    private static async Task<IResult> HandleGetSharedThread(
        string? token,
        IMediator mediator,
        ILogger<Program> logger,
        CancellationToken ct)
    ...
    private static async Task<IResult> HandleAddCommentToSharedThread(
        ...
        CancellationToken ct)
    {
        ...
        {
            logger.LogWarning(ex, "Comment denied: insufficient permissions");
            return Results.StatusCode(403); // Forbidden
        }
        catch (InvalidOperationException ex) when (ex.Message.Contains("Rate limit"))
        {
            logger.LogWarning(ex, "Comment denied: rate limit exceeded");
            return Results.StatusCode(429); // Too Many Requests
        }
        catch (ArgumentException ex)
        {
            logger.LogWarning(ex, "Comment validation failed: {Error}", ex.Message);
