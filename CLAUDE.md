...
**Mockup annotation pattern — DS-17-1 (#2069)** — every user-reachable `apps/web/src/app/.../page.tsx` carries a `@mockup` JSDoc block pointing to its `admin-mockups/design_files/*.html` reference. Inject via `pnpm mockup-annotations:inject --apply` (idempotent via `MOCKUP-ANNOTATION` marker); audit via `pnpm mockup-annotations:audit`. CI runs the audit as **non-blocking** report during Wave 2 (Top-30 pilot ≈ 12% coverage); flip to `--threshold 80` blocking when the sweep PR lands. Don't hand-edit the JSDoc block — fix `admin-mockups/MOCKUPS_INDEX.md` and re-run the injector. Pattern docs: [`docs/for-developers/frontend/mockup-annotation-pattern.md`](./docs/for-developers/frontend/mockup-annotation-pattern.md).
