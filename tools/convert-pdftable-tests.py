#!/usr/bin/env python3
"""
FluentAssertions Migration Script for PdfTableExtractionServiceTests.cs
Converts xUnit Assert.* patterns to FluentAssertions syntax

Phase 6 Target: 73 assertions in PdfTableExtractionServiceTests.cs
Distribution: Equal(35), Contains(20), True(7), False(5), Single(4)
Simple patterns - high automation expected (85%+)
"""
...
def main():
    file_path = "tests/Api.Tests/PdfTableExtractionServiceTests.cs"
    ...
    if content != original_content:
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"\n[SUCCESS] Successfully migrated {total_conversions} assertions in PdfTableExtractionServiceTests.cs")
        return 0
    else:
