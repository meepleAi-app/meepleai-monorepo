...
public sealed class GameBook
{
    ...
    // A5: nullable byte[]? mirrors the proven RowVersion pattern (SharedGameEntity, SessionEntity,
    // RuleSpecEntity, et al.); EF + Npgsql cannot insert a non-nullable bytea row_version without an
    // explicit value, since there is no DB-side default/trigger generated by either migrations or
    // EnsureCreatedAsync. Treating the column as nullable lets EF omit it from INSERT and rely on
    // PostgreSQL to populate it on the first UPDATE round-trip. [Timestamp] attribute signals the
    // setter is EF-reflection-driven (suppresses Sonar S1144 false-positive on the private setter).
    [Timestamp]
