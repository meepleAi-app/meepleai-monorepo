...
| 5 | Audit attribution: subject e actor nel posto giusto (D-S2-3) | ✅ *(production-pipeline gap chiuso in code-review)* | 2026-05-26 | Scenario S2-2 PASS + 3 nuovi unit test in `ValidateSessionQueryImpersonationTests` per la pipeline reale (5/5 PASS). **Bug scoperto in code-review post-T9 e corretto in-PR**: `ValidateSessionQueryHandler:105` aveva `Actor: null` hardcoded (il commento prometteva "T4 populates Actor" ma T4 toccava solo il ramo invalid). Gli scenari T8 passavano solo perché il fixture costruiva `SessionStatusDto` con Actor manuale, bypassando l'handler. Conseguenze in produzione (prima del fix): `audit_logs.impersonated_user_id` mai scritto (rompe D-S2-3), `EffectiveActor=Subject` durante impersonate (admin 403 su `/admin/impersonation/end`, rompe D-S2-1). Fix: handler carica l'admin via `_userRepository.GetByIdAsync(session.ImpersonatedByUserId)` con graceful-degrade ad Actor=null se l'admin è stato eliminato; `AdminImpersonationEndpoints.cs` 3 call site (`Start/End/Revoke`) riallineati a `EffectiveActor.Id`. |
...
4. **Actor-null bug (criteri 5+1, scoperto post-T9 in code-review e corretto in-PR)**: `Principal.Actor` era `null` hardcoded nel ramo valid di `ValidateSessionQueryHandler` — gli scenari T8 mascheravano il bug con un fixture manuale di `SessionStatusDto`. Fix shippato in questa PR (handler + 3 call site endpoint + 3 unit test sulla pipeline reale). Gate procedurale: la PR S3 (strict 2FA cutover) dovrebbe aggiungere un integration test end-to-end `HTTP → middleware → ValidateSession → behavior → audit_outbox` per evitare gap simili.

**S2 code-complete: 2026-05-26 by @DegrassiAaron — 8/8 DoD ✅ (criterio 7 benchmark p95 differito a ops; criteri 3+8 con deviazione documentata; criterio 5 con gap pipeline reale chiuso post-code-review).**
