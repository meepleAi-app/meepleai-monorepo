...
public sealed class RagServicePerformanceTests : IDisposable
{
    ...
    private IHybridSearchService CreateMockHybridSearchService()
    {
        ...
        mock.Setup(s => s.SearchAsync(
                ...
                It.IsAny<CancellationToken>()))
            .Returns(async (string query, Guid gameId, SearchMode mode, int limit, List<Guid>? documentIds, float vw, float kw, double minScore, GameBookRole roleHint, QueryEmbedding? precomputed, CancellationToken ct) =>
            {
