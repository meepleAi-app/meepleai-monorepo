...
**Victory carve-out (the one section without per-item JSONPath):** `ParseVictory` splits `$.victory` into a primary claim + N alternative claims that all necessarily share the anchor `$.victory`, and the guardrails only ever visit the `$.victory` *object* (alternatives are bare strings, never walked by the object-walker; T2/T3a/T3b key off `primary`/`citations`). So a Victory violation cannot be attributed per-alternative by path. **Accepted approximation (plan-reviewed, 2026-07-10):** since all Victory claims share the `$.victory` anchor, a Victory violation prefix-matches ALL Victory claims identically (primary + alternatives get the same outcome). A Victory `fail` badge means "the Victory section's primary failed"; the alternatives may show the same badge as a documented approximation. Every OTHER section (Summary/Mechanics/Resources/Phases/FAQ) gets **true per-claim precision** via the source-index anchor — that is the headline guarantee. A **precision follow-up** (distinguish primary via a `$.victory.alternatives[i]` anchor so the prefix-match naturally excludes alternatives) is filed to eliminate the Victory approximation later. `reject-all-failing-T2` is precise on all 5 non-Victory sections; on Victory a T2 fail (rare — keys off the primary text) would also select the section's alternatives.
