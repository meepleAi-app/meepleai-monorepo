...
export function FocusedCard(props: MeepleCardProps) {
  const { entity, title, subtitle, imageUrl, rating, ratingMax, metadata = [], actions = [], onClick } = props;
  const color = entityHsl(entity);

  return (
    <div className="flex flex-1 flex-col overflow-hidden rounded-[14px] border border-[var(--mc-border)] bg-[var(--mc-bg-card)] shadow-[var(--mc-shadow-xl)] outline-2 outline-offset-2 outline-transparent backdrop-blur-[12px]" onClick={onClick}>
      ...
      {imageUrl && (
        <div className="relative flex-shrink-0 overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 h-1/2" style={{ background: 'linear-gradient(to top, var(--mc-bg-card), transparent)' }} />
          <span className="absolute left-2 top-2 rounded-2xl px-2 py-0.5 font-[var(--font-quicksand)] text-[8px] font-bold uppercase tracking-wide text-white backdrop-blur-lg" style={{ background: entityHsl(entity, 0.8) }}>{entityLabel[entity]}</span>
          {rating !== undefined && (
            <span className="absolute right-2 top-2 flex items-center gap-[3px] rounded-[10px] bg-black/50 px-2 py-[3px] text-[10px] font-bold text-amber-300 backdrop-blur-lg">
              ★ {rating.toFixed(1)}{ratingMax && <span className="text-white/60">/{ratingMax}</span>}
            </span>
          )}
        </div>
      )}
      <div className="flex flex-1 flex-col gap-[5px] overflow-y-auto p-2.5 px-3">
        ...
        {subtitle && <p className="text-[0.72rem] leading-tight text-[var(--mc-text-secondary)]">{subtitle}</p>}
