...
public class TotpService : ITotpService
{
    ...
    /// </summary>
    private async Task TrackFailedAttemptAsync(Guid userId, string attemptType, CancellationToken ct = default)
    {
    ...
    /// </summary>
    private async Task<bool> IsAccountLockedOutAsync(Guid userId, string attemptType, CancellationToken ct = default)
    {
        var redisKey = $"2fa:failed:{attemptType}:{userId}";
        var redisDb = _redis.GetDatabase();
    ...
    /// </summary>
    private async Task ClearFailedAttemptsAsync(Guid userId, string attemptType, CancellationToken ct = default)
    {
        var redisKey = $"2fa:failed:{attemptType}:{userId}";
        var redisDb = _redis.GetDatabase();
