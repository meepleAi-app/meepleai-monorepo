...
import { createMockPdfDocument, createMockPdfList, mockId, HANDLER_BASE } from '../data/factories';
...
export const documentsHandlers = [
  ...
  http.post(`${API_BASE}/api/v1/ingest/pdf`, async ({ request }) => {
    ...
    if (file.size > maxSize) {
      return HttpResponse.json({ error: 'File too large. Maximum size is 10MB.' }, { status: 413 });
    }
