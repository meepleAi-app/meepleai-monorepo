...
{
#pragma warning disable S1075 // URIs should not be hardcoded - Default fallback for OpenAI public API
    private const string DefaultListModelsUrl = "https://api.openai.com/v1/models";
#pragma warning restore S1075
    ...
    public OpenAiProbeExecutor(
        IHttpClientFactory httpClientFactory,
        IConfiguration configuration,
        ILogger<OpenAiProbeExecutor> logger)
    {
        _httpClient = httpClientFactory.CreateClient("provider-probe");
        _logger = logger;
        _listModelsUrl = configuration["Providers:OpenAi:ListModelsUrl"] ?? DefaultListModelsUrl;
    }
    ...
    public async Task<ProbeExecutionResult> ExecuteAsync(string apiKey, string? expectedModel, CancellationToken cancellationToken)
    {
        ...
        {
            sw.Stop();
            _logger.LogWarning(ex, "OpenAI probe network failure");
