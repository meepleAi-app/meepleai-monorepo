/**
 * Accessibility tests — /players/[id] (Wave 3 Tier M, Issue #683).
 *
 * Combines:
 *   - axe-core WCAG 2.1 AA scan su default state (player detail populato via
 *     visual-test fixture — hero + stats grid + leaderboard + favorite agent
 *     + achievement grid)
 *   - axe-core WCAG 2.1 AA scan su not-found state (per coprire CTA + empty
 *     surface markup, sezioni non presenti su default)
 *   - prefers-reduced-motion contract: verifica che la route `/players/[id]`
 ...
 *     Il back button in PlayerHero usa `transition-colors` — deve collassare a
 *     sub-millisecondo durations.
 *
 * Comprehensive unit-level coverage del 4-state FSM + ?state= override matrix
 * lives in `_components/__tests__/PlayerDetailView.test.tsx`.
 ...
 * venga short-circuitato dal fixture deterministico (Sara Rossi profile,
 * Wingspan-shaped).
 *
 ...
 * I componenti player-detail v2 sono pure components NON basati su MeepleCard —
...
async function gotoPlayerDetailReady(page: Page, search = ''): Promise<void> {
  await seedAuthSession(page);
  await seedCookieConsent(page);
  await mockAuthEndpoints(page);
  await page.goto(`/players/${FIXTURE_PLAYER_ID}${search}`, { waitUntil: 'domcontentloaded' });
...
test.describe('Player detail — accessibility @a11y', () => {
  test('axe-core: no WCAG 2.1 AA violations on default state', async ({ page }) => {
    await gotoPlayerDetailReady(page);
    // Default state: hero + stats grid visible from visual-test fixture
    await expect(page.locator('[data-slot="player-detail-hero"]')).toBeVisible();
    await expect(page.locator('[data-slot="player-detail-stats-grid"]')).toBeVisible();
  ...
  test.describe('prefers-reduced-motion contract', () => {
    ...
    test('back button transition collapses under reduced-motion', async ({ page }) => {
      ...
      // `@media (prefers-reduced-motion: reduce)`. PlayerHero back button uses
      // `transition-colors` — this collapses to instant under reduced-motion.
      await page.emulateMedia({ reducedMotion: 'reduce' });
      await gotoPlayerDetailReady(page);
      await expect(page.locator('[data-slot="player-detail-hero"]')).toBeVisible();

      // PlayerHero back button (data-slot="player-detail-hero-back").
      // Under reduced-motion the global CSS override forces
      // `transition-duration: 0.01ms !important`.
      const backButton = page.locator('[data-slot="player-detail-hero-back"]');
      await expect(backButton).toBeAttached({ timeout: 10_000 });

      const durationMs = await backButton.evaluate(el => {
        const cs = window.getComputedStyle(el as HTMLElement);
        // transition-duration may be a comma-separated list (e.g. "0.01ms, 0.01ms").
