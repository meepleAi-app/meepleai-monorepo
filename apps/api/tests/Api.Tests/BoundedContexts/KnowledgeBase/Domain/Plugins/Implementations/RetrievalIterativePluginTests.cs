...
// Issue #3358 - Iterative RAG Strategy Tests
// =============================================================================
...
/// <summary>
/// Unit tests for RetrievalIterativePlugin.
/// Issue #3358: Iterative RAG Strategy implementation.
/// </summary>
[Trait("Category", "Unit")]
[Trait("BoundedContext", "KnowledgeBase")]
[Trait("Component", "Plugins")]
public class RetrievalIterativePluginTests
{
    private readonly RetrievalIterativePlugin _sut;
    private readonly Mock<ILogger<RetrievalIterativePlugin>> _loggerMock;

    public RetrievalIterativePluginTests()
    {
        _loggerMock = new Mock<ILogger<RetrievalIterativePlugin>>();
        _sut = new RetrievalIterativePlugin(_loggerMock.Object);
    }
    ...
    {
        // Assert
        _sut.Id.Should().Be("retrieval-iterative-v1");
    }
    ...
    {
        // Assert
        _sut.Name.Should().Be("Iterative Retrieval");
    }
    ...
    public void Plugin_ShouldHaveValidConfigSchema()
    {
        ...
        schema.Should().NotBeNull();
        schema.RootElement.TryGetProperty("properties", out var propsElement).Should().BeTrue();
        propsElement.TryGetProperty("maxIterations", out _).Should().BeTrue();
        propsElement.TryGetProperty("topK", out _).Should().BeTrue();
        propsElement.TryGetProperty("minScore", out _).Should().BeTrue();
        propsElement.TryGetProperty("refinementThreshold", out _).Should().BeTrue();
    }
    ...
    public async Task ExecuteAsync_ShouldReturnMetricsInResult()
    {
        ...
        metrics.TryGetProperty("iterationsPerformed", out _).Should().BeTrue();
        metrics.TryGetProperty("totalDocumentsRetrieved", out _).Should().BeTrue();
        metrics.TryGetProperty("documentsAfterDedup", out _).Should().BeTrue();
        metrics.TryGetProperty("refinedQueries", out _).Should().BeTrue();
        metrics.TryGetProperty("totalTokens", out _).Should().BeTrue();
        metrics.TryGetProperty("averageScore", out _).Should().BeTrue();
    }

    [Fact]
    public async Task ExecuteAsync_ShouldRecordIterationCount()
    {
        // Arrange
        var input = CreateValidInput("Complex multi-hop question about game mechanics");
        var config = PluginConfig.Default();
        ...
        result.Success.Should().BeTrue();
        result.Result!.RootElement.TryGetProperty("metrics", out var metrics).Should().BeTrue();
        metrics.TryGetProperty("iterationsPerformed", out var iterations).Should().BeTrue();
        iterations.GetInt32().Should().BeGreaterThanOrEqualTo(1);
    }
    ...
    [Fact]
    public async Task ExecuteAsync_WithCustomMaxIterations_ShouldUseConfiguredValue()
    {
        // Arrange
        var input = CreateValidInput("Test query");
        var customConfig = JsonDocument.Parse("""{"maxIterations": 2}""");
        ...
        result.Result!.RootElement.TryGetProperty("metrics", out var metrics).Should().BeTrue();
        metrics.TryGetProperty("iterationsPerformed", out var iterations).Should().BeTrue();
        iterations.GetInt32().Should().BeLessThanOrEqualTo(2);
    }
    ...
    #region Refined Queries Tests

    [Fact]
    public async Task ExecuteAsync_ShouldRecordRefinedQueries()
    {
        // Arrange
        var input = CreateValidInput("Complex question about game strategy");
        var config = PluginConfig.Default();
        ...
        result.Success.Should().BeTrue();
        result.Result!.RootElement.TryGetProperty("metrics", out var metrics).Should().BeTrue();
        metrics.TryGetProperty("refinedQueries", out var queries).Should().BeTrue();
        queries.GetArrayLength().Should().BeGreaterThanOrEqualTo(1);
    }

    [Fact]
    public async Task ExecuteAsync_RefinedQueriesShouldIncludeOriginalQuery()
    {
        // Arrange
        var originalQuery = "How do I play this game?";
        ...
        var firstQuery = queries.EnumerateArray().First().GetString();
        firstQuery.Should().Be(originalQuery);
    }
