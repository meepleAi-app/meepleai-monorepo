...
PlayRecordSummaryDto (for history)
...
export const playSessionsApi = {
  ...
  getSession: (sessionId: string) => Promise<PlayRecordDto>,
...
```typescript
interface PlayRecordStore {
  currentSession: PlayRecordDto | null;
...
```bash
cd apps/api/src/Api
dotnet ef migrations add AddPlayRecords
...
- [Play Records API Docs](../../api/bounded-contexts/game-management/play-sessions.md)
