...
/**
 * MeepleCardHero - Hero variant renderer
 *
 * Full-width, min-h-320px, absolute inset cover image,
 * content overlaid at bottom with gradient.
 *
 * @module components/ui/data-display/meeple-card/variants/MeepleCardHero
 */
...
export type MeepleCardHeroProps = MeepleCardProps;

/**
 * Hero variant: full-width card with absolute inset cover, content overlay at bottom.
 * min-h-320px, entity ribbon indicator, hero-specific text styling.
 */
export const MeepleCardHero = React.memo(function MeepleCardHero(props: MeepleCardHeroProps) {
  const {
    id,
    entity,
    title,
  ...
  return (
    ...
    >
      ...
      <div className={contentVariants({ variant })}>
        ...
        {subtitle && <p className="text-white/85 text-sm mb-2">{subtitle}</p>}

        {rating !== undefined && (
          <RatingDisplay rating={rating} max={ratingMax} className="mb-2 text-white" />
        )}
        ...
        {/* Metadata (hero: mt-auto) */}
        {metadata.length > 0 && (
          <MetadataChips metadata={metadata} variant={variant} className="mt-auto" />
        )}
