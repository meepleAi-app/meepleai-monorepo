...
| D2 | Concurrency (two requests resolving the same NEW designer/publisher name concurrently → UNIQUE violation) | **Mirror the seeder** — no concurrency handling; document the race (re-confirmed after code review) | Corrected framing (post code-review): `AddAsync` is hit by several concurrent request paths (`CreateSharedGameCommand`, `ApproveGameProposal`, promotion), not only the low-frequency promotion. The failure mode is nonetheless **benign**: the UNIQUE index rejects the duplicate (no data corruption), the losing request gets a 500 and is **retryable** (on retry the name exists → succeeds). Accepted as a known limitation; robust handling (retry on Postgres `23505` / `ON CONFLICT`) deferred. |
