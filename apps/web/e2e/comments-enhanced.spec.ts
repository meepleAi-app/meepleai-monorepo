...
test.describe('EDIT-05: Enhanced Comments System', () => {
  ...
  const loginAs = async (page: any, email: string, password: string = 'Demo123!') => {
    await page.goto('/');
  ...
  test.beforeEach(async ({ page }) => {
    // Login as editor (can create comments)
    await loginAs(page, 'editor@meepleai.dev');

    // Navigate to versions page (where comments are displayed)
    await page.goto(VERSIONS_URL);

    // Wait for page to load
    await expect(page.locator('h1')).toContainText('Storico Versioni');
  });

  test('should display comments section on versions page', async ({ page }) => {
  ...
  test('should create top-level comment successfully', async ({ page }) => {
  ...
  test('should create threaded reply successfully', async ({ page }) => {
  ...
  test('should resolve comment successfully', async ({ page }) => {
  ...
  test('should unresolve comment successfully', async ({ page }) => {
  ...
  test('should filter resolved comments when toggling checkbox', async ({ page }) => {
  ...
  test('should show edit/delete buttons only for own comments', async ({ page }) => {
  ...
  test('should show cancel button when replying and cancel reply', async ({ page }) => {
  ...
  test.describe('Admin Permissions', () => {
    test('admin can delete any comment', async ({ page }) => {
      ...
      // Act: Login as admin
      await loginAs(page, 'admin@meepleai.dev');
      await page.goto(VERSIONS_URL);
  ...
  test.describe('User Role Restrictions', () => {
    test('regular user cannot create comments', async ({ page }) => {
      ...
      await page.goto('/');
      await page.click('button:has-text("Logout")');
      await loginAs(page, 'user@meepleai.dev');
