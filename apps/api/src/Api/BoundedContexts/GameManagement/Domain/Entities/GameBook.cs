...
public sealed class GameBook
{
    ...
    // #3651: token di concorrenza ottimistica sulla colonna di sistema `xmin` (ADR-060).
    //
    // Era `[Timestamp] byte[]? RowVersion` su una colonna `bytea`, con l'assunzione — scritta nel
    // commento precedente — di poter «rely on PostgreSQL to populate it on the first UPDATE
    // round-trip». Postgres non lo fa: una `bytea` la popola solo chi la scrive, e il trigger che
    // se ne occupava è stato rimosso da #2305. Il token restava quindi NULL su ogni riga, EF
    // confrontava `NULL = NULL` a ogni update e due editor concorrenti sullo stesso libro si
    // sovrascrivevano a vicenda senza alcun segnale.
    //
    // `xmin` è di sistema: Postgres la valorizza da sé a ogni UPDATE, e non compare mai in un
    // INSERT — quindi cade anche la ragione per cui la colonna doveva essere nullable.
    //
    // Get-only: il dominio non la assegna mai, la scrive solo EF Core sul backing field. Un
    // `private set` verrebbe segnalato da S1144 come setter inutilizzato — è la ragione per cui
    // la versione precedente portava `[Timestamp]`, che sopprimeva il falso positivo.
