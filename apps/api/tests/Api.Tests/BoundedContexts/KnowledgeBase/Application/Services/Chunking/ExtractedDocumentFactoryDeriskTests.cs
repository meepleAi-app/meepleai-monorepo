...
/// (fixture captured via `curl .../api/v1/extract` against the live Docker service, not fabricated),
...
///
/// Fixture: apps/api/tests/Api.Tests/TestData/unstructured-terraforming-response.json — captured from
/// `terraforming-mars_rulebook.pdf` (the game that motivated the epic) via `strategy=fast`,
/// `language=ita` against the local unstructured-service (localhost:8001). Detected language: "ita".
/// Raw element categories found: Title (82), NarrativeText (120), UncategorizedText (47), Footer (24),
/// Header (5), ListItem (29) — out of 307 total elements. The 6 headings below are real, standalone
/// "Title"-category section headings verified present verbatim in the captured fixture (CONTESTO on
/// p2, PARAMETRI GLOBALI on p3, GENERAZIONI on p8, AZIONI on p9, FINE DEL GIOCO on p12, VARIANTI on
/// p13) — real rulebook section boundaries (context/setup, global-parameters mechanic, round
/// structure, turn actions, end-of-game, solo/alternate-rules variants).
/// </summary>
