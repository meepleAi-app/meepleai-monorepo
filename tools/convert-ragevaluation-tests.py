#!/usr/bin/env python3
"""
FluentAssertions Migration Script for RagEvaluationServiceTests.cs
Converts xUnit Assert.* patterns to FluentAssertions syntax

Phase 9 Target: 65 assertions in RagEvaluationServiceTests.cs
Distribution: Equal(34), Contains(14), ThrowsAsync(7), True(4), False(3), Throws(2), Single(1)
...
def main():
    file_path = "tests/Api.Tests/RagEvaluationServiceTests.cs"
    ...
    if content != original_content:
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"\n[SUCCESS] Successfully migrated {total_conversions} assertions in RagEvaluationServiceTests.cs")
        return 0
    else:
