...
/// <summary>
/// Orchestrates the full playground chat pipeline: RAG retrieval, strategy-based LLM
/// execution (RetrievalOnly / SingleModel / MultiModelConsensus), cost tracking, and
/// telemetry assembly (pipeline timings, API traces, TOMAC layers, data flow steps).
///
/// Extracted from PlaygroundChatCommandHandler to keep the handler thin.
/// Issue #4392, #4437, #4443, #4446, #4456, #4458.
/// </summary>
internal sealed class PlaygroundChatOrchestrator : IPlaygroundChatOrchestrator
{
    private static readonly string[] DefaultConsensusModels = { "gpt-4", "claude-3-opus" };

    /// <summary>
    /// In-memory query cache shared across all orchestrator instances (static = process-wide).
    /// Key: SHA256(gameId + query), Value: (resultCount, cachedAt).
    /// Used for cache observability in debug panel (Issue #4443).
    /// </summary>
    private static readonly ConcurrentDictionary<string, PlaygroundCacheEntry> QueryCache
    ...
    private readonly ILogger<PlaygroundChatOrchestrator> _logger;

    public PlaygroundChatOrchestrator(
        ...
        ILogger<PlaygroundChatOrchestrator> logger)
    {
...
#pragma warning disable S4456 // Standard async-enumerable streaming pattern
    public async IAsyncEnumerable<RagStreamingEvent> ExecuteAsync(
...
#pragma warning restore S4456
    {
        ...
        await foreach (var @event in ExecuteCore(request, cancellationToken).ConfigureAwait(false))
        {
            yield return @event;
        }
    }

    private async IAsyncEnumerable<RagStreamingEvent> ExecuteCore(
        PlaygroundChatRequest request,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        var effectiveStrategy = ResolveStrategy(request.Strategy);

        // Data flow step 1: Query Input (Issue #4456)
        dataFlowSteps.Add(new PlaygroundDataFlowStep(
            "Query Input", "query",
            $"User query ({request.Message.Length} chars)",
            new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ["rawQuery"] = request.Message.Length > 300 ? request.Message[..300] + "..." : request.Message,
                ["strategy"] = effectiveStrategy,
                ["gameContext"] = request.GameId.HasValue ? request.GameId.Value.ToString("D", CultureInfo.InvariantCulture) : "none",
            }));

        Log("info", "Pipeline", $"Starting playground chat for agent {request.AgentDefinitionId}");
        Log("debug", "Strategy", $"Resolved strategy: {effectiveStrategy} (requested: {request.Strategy ?? "null"})");

        _logger.LogInformation(
            "Playground chat starting for AgentDefinition {AgentDefinitionId} with strategy {Strategy}",
            request.AgentDefinitionId, effectiveStrategy);
        ...
        if (agentDefinition == null)
        {
            yield return CreateEvent(
                ...
                new StreamingError(
                    $"AgentDefinition {request.AgentDefinitionId} not found",
                    "AGENT_NOT_FOUND"));
            yield break;
        }
        ...
        if (request.GameId.HasValue)
        {
            ...
            var cacheKey = BuildCacheKey(request.GameId.Value, request.Message);
            ...
            var searchRequestSize = Encoding.UTF8.GetByteCount(request.Message);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Hybrid search failed for game {GameId}", request.GameId.Value);
                ...
                apiTraces.Add(new PlaygroundApiTrace(
                    ...
                    requestPreview: request.Message.Length > 500 ? request.Message[..500] : request.Message,
        ...
        switch (effectiveStrategy)
        {
            ...
            case "MultiModelConsensus":
                ...
                foreach (var model in models)
                {
                    ...
                    var consensusLlmStopwatch = System.Diagnostics.Stopwatch.StartNew();
            ...
            default: // "SingleModel" or any other - standard flow
                ...
                var singleLlmStopwatch = System.Diagnostics.Stopwatch.StartNew();
        ...
        var hasRag = request.GameId.HasValue && ragSnippets is { Count: > 0 };
        ...
        _logger.LogInformation(
            ...
            request.AgentDefinitionId, effectiveStrategy, totalTokens, costCalculation.TotalCost, totalStopwatch.ElapsedMilliseconds);
    ...
    private static string BuildSystemPrompt(AgentDefinition agentDefinition)
    {
