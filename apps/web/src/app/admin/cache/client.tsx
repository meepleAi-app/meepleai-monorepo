    ...
    setConfirmation({
      ...
      onConfirm: async () => {
        try {
          ...
          addToast(
            'error',
            `Failed to invalidate cache: ${err instanceof Error ? err.message : 'Unknown error'}`
          );
          setConfirmation({ ...confirmation, isOpen: false });
        }
      },
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-4 mb-6">
            ...
            <div className="p-6 border border-gray-300 rounded-lg bg-white">
              ...
              <div
                ...
                style={{ color: getHitRateColor(stats.hitRate ?? 0) }}
              >
            ...
            <div className="p-6 border border-gray-300 rounded-lg bg-white">
              ...
              <div className="text-3xl font-semibold">
                {formatCacheSize(stats.cacheSizeBytes ?? 0)}
              </div>
              <div className="mt-3 text-sm text-gray-500">{stats.totalKeys ?? 0} cached keys</div>
            </div>
          </div>
        )}
          ...
          <div className="border border-gray-300 rounded-lg overflow-hidden">
            ...
            {stats.topQuestions.map((question, index) => (
              <div
                ...
                style={{ gridTemplateColumns: '1fr 120px 200px' }}
              >
                <div className="overflow-hidden text-ellipsis">{question.questionHash}</div>
                <div className="font-semibold text-blue-600">{question.hitCount}</div>
