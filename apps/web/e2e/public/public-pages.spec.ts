...
test.describe('Catalog Giochi', () => {
  ...
  test('ricerca filtra giochi', async ({ page }) => {
    ...
    const searchInput = page
      .locator('input[placeholder*="erca"], input[type="search"], input[name="search"]')
      .first();
  ...
  test('click su gioco naviga al dettaglio', async ({ page }) => {
    ...
    const gameCard = page.locator('[href*="/library/games/"]').first();
    if ((await gameCard.count()) > 0) {
      const href = await gameCard.getAttribute('href');
      if (href && /\/library\/games\/.+/.test(href)) {
        await gameCard.click();
        await page.waitForURL(/\/library\/games\/[^/]+/, { timeout: 8000 }).catch(() => {});
      }
    }
  });
});

test.describe('Dettaglio Gioco', () => {
  test.beforeEach(async ({ page }) => {
  ...
  test('mostra titolo e dati base', async ({ page }) => {
    await page.goto('/library/games/g1', { waitUntil: 'domcontentloaded' });
  ...
  test('mostra lista sessioni', async ({ page }) => {
    await page.route(`${API_BASE}/api/v1/sessions**`, route =>
      route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          items: [{ id: 's1', gameTitle: 'Catan', date: '2026-02-20', players: 3 }],
          totalCount: 1,
        }),
      })
    );
