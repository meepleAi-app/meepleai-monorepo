/**
 * Accessibility tests — /agents/[id] (Wave C.2, Issue #581).
 *
 * Combines:
 *   - axe-core WCAG 2.1 AA scan su default state (agent detail populated via
 *     visual-test fixture — hero + 5 tabs + identity panel)
 *   - axe-core WCAG 2.1 AA scan su not-found state (per coprire CTA + empty
 *     surface markup, sezioni non presenti su default)
 *   - prefers-reduced-motion contract: verifica che la route `/agents/[id]`
 ...
 *     La tabs animated underline (AgentTabs `motion-safe:transition-all
 *     motion-safe:duration-300`) deve collassare a sub-millisecondo.
 *
 * Comprehensive unit-level coverage del FSM + ?state= override matrix
 * lives in `_components/__tests__/AgentDetailViewV2.test.tsx` (20 tests).
 ...
 * `NEXT_PUBLIC_VISUAL_TEST_FIXTURE_ENABLED=1` in modo che `useAgent`
 * venga short-circuitato dal fixture deterministico (Catan-shaped agent data).
 *
 * A11y CTA contrast: Task 2 ha preemptively usato 700-shade Tailwind:
 *   - AgentHero active CTA: `bg-emerald-700` (not 600)
 *   - AgentHero draft CTA: `bg-amber-700` (not 600)
 *   - AgentHero archived CTA: `bg-violet-700` (not 600)
 *   - NotFoundShell CTA: `bg-violet-700` (not 600)
 * axe dovrebbe passare clean. Se unexpected violations dalla MeepleCard-family
 * (EntityBadge / StatusBadge), aggiungere `.exclude('[data-slot="..."]')` chain
 * con rationale comment e estendere Issue #636 (Wave B.1/B.2 audit-driven pattern).
 * I componenti agent-detail v2 sono pure components NON basati su MeepleCard —
 * le esclusioni non sono preemptively aggiunte qui per non mascherare nuove violazioni.
 *
 * Cherry-picked from Wave C.1 a11y/game-detail.spec.ts — data-slot selectors updated to
 * match Task 2/3 committed components. Reduced-motion test simplified to inspect
 * the agent-detail-tab-indicator element directly.
 */
...
async function gotoAgentDetailReady(page: Page, search = ''): Promise<void> {
  await seedAuthSession(page);
  await seedCookieConsent(page);
  await mockAuthEndpoints(page);
  await page.goto(`/agents/${FIXTURE_AGENT_ID}${search}`, { waitUntil: 'domcontentloaded' });
...
test.describe('Agent detail — accessibility @a11y', () => {
  test('axe-core: no WCAG 2.1 AA violations on default state', async ({ page }) => {
    await gotoAgentDetailReady(page);
    // Default state: hero + tablist visible from visual-test fixture
    await expect(page.locator('[data-slot="agent-detail-hero"]')).toBeVisible();
    await expect(page.locator('[data-slot="agent-detail-tabs"]')).toBeVisible();
  ...
  test('axe-core: no WCAG 2.1 AA violations on not-found state', async ({ page }) => {
    ...
    // Not-found shell: CTA + back-to-agents link
  ...
  test.describe('prefers-reduced-motion contract', () => {
    ...
    test('tabs underline animation collapses under reduced-motion', async ({ page }) => {
      ...
      // `@media (prefers-reduced-motion: reduce)`. AgentTabs uses
      ...
      // AgentTabs indicator (data-slot="agent-detail-tab-indicator").
      // Under reduced-motion the `motion-safe:transition-all motion-safe:duration-300`
      // class is inert, and the global CSS override forces `transition-duration: 0.01ms !important`.
      const indicator = page.locator('[data-slot="agent-detail-tab-indicator"]');
