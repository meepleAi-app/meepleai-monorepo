/**
 * E2E Test: PDF Preview Component - Real Backend Integration
 *
...
const authenticatedTest = test.extend<{ editorPage: Page }>({
...
authenticatedTest.describe('PDF Preview Component - Full User Journey', () => {
  authenticatedTest(
    'User previews PDF with zoom, navigation, and thumbnails before upload',
  ...
  authenticatedTest(
    'PDF preview handles keyboard navigation without interfering with input fields',
    async ({ editorPage: page }) => {
      // When: User navigates to upload page
      await page.goto('/upload');
      await expect(page.locator('h1')).toContainText('PDF Import Wizard');
  ...
  authenticatedTest(
    'PDF preview displays error for invalid PDF files',
    async ({ editorPage: page }) => {
      // When: User navigates to upload page
      await page.goto('/upload');
      await expect(page.locator('h1')).toContainText('PDF Import Wizard');
  ...
  authenticatedTest(
    'PDF preview zoom buttons have correct disabled states',
    async ({ editorPage: page }) => {
  ...
  authenticatedTest(
    'PDF preview page navigation buttons have correct disabled states',
    async ({ editorPage: page }) => {
      // Navigate to upload page
      await page.goto('/upload');
      await expect(page.locator('select#gameSelect')).toBeVisible({ timeout: 10000 });
