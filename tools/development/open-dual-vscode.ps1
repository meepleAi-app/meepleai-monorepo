...
if (-not (Test-Path $backendPath) -and -not $BackendOnly) {
    Write-Host "ERROR: Backend worktree not found at: $backendPath" -ForegroundColor Red
...
if (-not (Test-Path $frontendPath) -and -not $FrontendOnly) {
    Write-Host "ERROR: Frontend worktree not found at: $frontendPath" -ForegroundColor Red
...
Write-Host ""
Write-Host "SUCCESS: VS Code instances opened!" -ForegroundColor Green
Write-Host ""
