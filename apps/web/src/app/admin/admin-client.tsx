...
export function AdminClient() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Fetch requests
      const requestsData = await api.admin.getAiRequests({
        page,
        pageSize,
    ...
    <main className="p-6 font-sans max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="mt-2 mb-0 text-gray-500">Monitor AI requests, latency, and token usage</p>
        </div>
      ...
      {stats && requests.length > 0 && (
        <div className="grid grid-cols-[repeat(auto-fit,minmax(500px,1fr))] gap-4 mb-6">
          ...
          <div className="p-6 border border-gray-300 rounded-lg bg-white">
            ...
            <RequestsTimeSeriesChart
              requests={filteredRequests.map(r => ({ createdAt: r.createdAt, status: r.status }))}
            />
          </div>
            ...
            {Object.entries(stats.endpointCounts).map(([endpoint, count]) => (
              <div key={endpoint} className="flex-1">
                ...
                <div
                  ...
                  style={{ color: getEndpointColor(endpoint) }}
                >
                  {count}
                </div>
              </div>
      ...
      <div className="flex gap-3 mb-6">
        <div className="flex-1">
          <label
            ...
            className="block text-xs mb-1 text-gray-500 font-semibold"
          >
            Start Date
          </label>
          <input
      ...
      <div className="border border-gray-300 rounded-lg overflow-hidden">
        <div
          className="p-4 bg-gray-50 border-b border-gray-300 grid gap-4 text-xs font-semibold text-gray-500 uppercase"
          style={{
          ...
          filteredRequests.map(req => (
            ...
            >
              ...
              <div
                ...
                style={{ color: getEndpointColor(req.endpoint) }}
              >
                {req.endpoint}
              </div>
              ...
              <div className="text-xs font-mono text-gray-500">
                {req.confidence !== null && req.confidence !== undefined
                  ? req.confidence.toFixed(2)
                  : '-'}
              </div>
              <div className="text-xs text-gray-500">
                {req.model
                  ? `${req.model}${req.finishReason ? ` (${req.finishReason})` : ''}`
                  : '-'}
              </div>
