...
| #3740 | 🔴 **`PgVectorStoreAdapter.SearchWithScoresAsync` NON seleziona `e.lang`**, e `Embedding.Language` ha l'inizializzatore `= "en"`: ogni candidato del braccio vettoriale arriva quindi a `"en"` **qualunque sia la lingua vera del chunk**. Una colonna mancante mascherata da un default non-null — è il motivo per cui una correzione per lingua (#3743) è risultata un no-op byte-identico e sembrava che il corpus fosse monolingua. Non lo è: lo snapshot del gate ha **9840 en / 943 it / 107 de** su 13 manuali non inglesi. Prima di scrivere logica che legge `Embedding.Language` da quel percorso, **verifica che la colonna sia nella SELECT**. Corpora comunque distinti per granularità: gate 10.890 chunk (Docnet) vs staging 56.367 (heading-aware). Storia: [audit](./docs/for-developers/audits/2026-08-17-e5-prefix-and-cross-lingual-retrieval-audit.md) |
