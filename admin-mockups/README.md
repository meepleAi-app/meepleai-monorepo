...
The mockups are **wired together as a navigable demo**. To explore the full product surface:

```bash
cd admin-mockups/design_files
...
Navigation is generated by `scripts/mockup_demo/` (Python 3.11+ stdlib, no deps):

| File | Purpose |
|---|---|
| `build_map.py` | Scan mockups → emit `docs/superpowers/specs/nav-map.md` (mapping table) |
| `classify_todos.py` | Heuristic batch-classification of TODO rows into destinations or OUT_OF_SCOPE |
| `apply_map.py` | Read approved nav-map → patch HTML/JSX files in place |
| `validate.py` | BFS reachability + broken-target report → `docs/superpowers/specs/nav-validation-report.html` |

To regenerate after editing rules in `scripts/mockup_demo/data/canonical_nav.json`:

```bash
python -m scripts.mockup_demo.build_map
python -m scripts.mockup_demo.classify_todos --apply
python -m scripts.mockup_demo.apply_map
python -m scripts.mockup_demo.validate
```

Patches are idempotent (guarded by `/* DEMO-NAV */` marker — re-running `apply_map` produces no changes).

See:
- [Design doc](../docs/superpowers/specs/2026-05-20-mockup-demo-navigation-design.md)
- [Implementation plan](../docs/superpowers/plans/2026-05-20-mockup-demo-navigation.md)
- [Nav map](../docs/superpowers/specs/nav-map.md) — per-mockup destination table
