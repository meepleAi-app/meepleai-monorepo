...
internal class BggApiService : IBggApiService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var cachedResults = await _cache.GetOrCreateAsync<List<BggSearchResultDto>?>(
            ...
            async cancel =>
            {
                _logger.LogInformation("BGG search cache miss for query: {Query}", LogSanitizer.Sanitize(query));
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Fetching BGG search results from: {Url}", LogSanitizer.Sanitize(url));
            ...
            _logger.LogInformation("BGG search returned {Count} results for query: {Query}", results?.Count ?? 0, LogSanitizer.Sanitize(query));
            return results;
        }
        catch (HttpRequestException ex)
        {
            ...
            _logger.LogWarning(ex, "BGG API unavailable for search query: {Query}. Returning empty results.", LogSanitizer.Sanitize(query));
            return null;
        }
