...
const BLOCK_EXPLANATIONS: Record<string, { explanation: string; technical: string; code?: string }> = {
  'input': {
    explanation: 'Punto di ingresso per la query dell\'utente. Il sistema riceve la domanda e la classifica per complessità e tipo.',
    technical: 'La query viene preprocessata (lowercase, tokenizzazione) e viene estratto il contesto (GameId, ThreadId se conversazione esistente).',
    code: 'AskQuestionQueryHandler.cs:30-45'
  },
  'agent_general': {
    explanation: 'GeneralAgent è l\'agente predefinito per domande semplici. Fornisce risposte concise e dirette.',
    technical: 'BasePrompt: "You are a helpful assistant. Provide concise answers." DefaultStrategy: FAST. Ottimizzato per FAQ e lookup diretti.',
    code: 'AgentOrchestrationService.cs:150-160'
  },
  'agent_rules': {
    explanation: 'RulesInterpreter è specializzato nell\'interpretazione autoritativa delle regole di gioco. Fornisce citazioni precise dal regolamento.',
    technical: 'BasePrompt: "You are a rules expert. Provide authoritative interpretations with page references." DefaultStrategy: BALANCED. Triggered by keywords: rule, can i, is it legal.',
    code: 'AgentOrchestrationService.cs:140-149'
  },
  'model_llama': {
    explanation: 'Llama 3.3 70B è un modello open-source gratuito, ottimo per query semplici con zero costi.',
    technical: 'Provider: OpenRouter (free tier). Context: 4K tokens. Speed: ~50 tokens/sec. Accuracy: buona per FAQ, non ideale per ragionamento complesso.',
  ...
  'model_deepseek': {
    ...
    technical: 'Provider: DeepSeek. Cost: $0.28/M input, $0.42/M output. Context: 16K tokens. Speed: ~80 tokens/sec. Accuracy: competitiva con modelli premium.',
    code: 'HybridAdaptiveRoutingStrategy.cs:220-235'
  },
  'model_sonnet': {
    explanation: 'Claude Sonnet 4.5 è il modello premium per reasoning complesso e decisioni critiche.',
    technical: 'Provider: Anthropic. Cost: $3/M input, $15/M output. Context: 200K tokens. Speed: ~100 tokens/sec. Accuracy: state-of-the-art per ragionamento multi-step.',
    code: 'HybridAdaptiveRoutingStrategy.cs:250-265'
  },
  'layer_routing': {
    explanation: 'L1 Routing classifica la query e valida che il tier dell\'utente abbia accesso alla strategia selezionata.',
    technical: 'Usa LLM lightweight per classificare QueryType (RulesInterpretation, Citation, Strategy, General). Verifica TierStrategyAccess. Tokens: 280-360. Latency: 20-50ms.',
    code: 'AgentOrchestrationService.cs:23-69'
  },
  'layer_cache': {
    explanation: 'L2 Cache controlla se esiste una risposta simile già calcolata, evitando retrieval e generation non necessari.',
    technical: 'Semantic similarity search in Redis. Threshold: cosine > 0.95. Hit rate target: 80%. Se hit, risparmia ~2000 tokens. TTL varia per tier (48h User, 168h Admin).',
    code: 'SemanticCacheDomainService.cs:40-85'
  },
  'layer_retrieval': {
    explanation: 'L3 Retrieval cerca nel database vettoriale i chunk più rilevanti del regolamento del gioco.',
    technical: 'Vector search in Qdrant con cosine similarity. TopK varia (3 per FAST, 5 per BALANCED). MinScore: 0.55. Genera embedding della query e cerca i chunk più simili.',
    code: 'VectorSearchDomainService.cs:20-65'
  },
  'layer_crag': {
    explanation: 'L4 CRAG valuta la qualità dei documenti recuperati e decide se sono sufficienti o se serve ricerca aggiuntiva.',
    technical: 'CRAG Evaluator (T5-Large) assegna score: Correct (>0.8), Ambiguous (0.5-0.8), Incorrect (<0.5). Se Incorrect → trigger web search o query rewriting.',
    code: 'CragEvaluationService.cs:55-120'
  },
  'layer_generation': {
    explanation: 'L5 Generation sintetizza la risposta finale usando il modello LLM selezionato e il contesto recuperato.',
    technical: 'Costruisce prompt: SystemPrompt (agent BasePrompt) + UserPrompt (query + retrieved chunks). Chiama LLM con temperature 0.7, max_tokens 1000. Traccia token usage con OpenTelemetry.',
    code: 'AskQuestionQueryHandler.cs:94-120'
  },
  'layer_validation': {
    explanation: 'L6 Validation esegue 5 layer di controllo qualità: confidence, multi-model, citation, hallucination, accuracy tracking.',
    technical: 'Pipeline: V1 (confidence ≥0.70) → V2 (multi-model similarity ≥0.90) + V3 (citation in PDF) → V4 (no forbidden words) → V5 (accuracy tracking). Parallel execution di V2+V3.',
    code: 'RagValidationPipelineService.cs:55-259'
  },
  'layer_multihop': {
    explanation: 'Multi-Hop Retrieval esegue retrieval iterativo: usa risultati del hop precedente per raffinare la ricerca successiva.',
    technical: 'Strategia: 1) Initial retrieval (TopK=5), 2) Extract entities from chunks, 3) Search entities (TopK=3), 4) Merge results with RRF fusion. Tipicamente 2-3 hops.',
    code: 'MultiHopRetrievalService.cs:30-95 (future implementation)'
  },
  'layer_multiagent': {
    explanation: 'Multi-Agent Pipeline coordina 3 agenti specializzati: Analyzer, Validator, Synthesizer lavorano in sequenza.',
    technical: 'LangGraph orchestration: Agent1 analizza query → Agent2 valida sources → Agent3 sintetizza risposta finale. Ogni agente usa diverso modello (Haiku, Sonnet, Opus) progressivamente.',
    code: 'MultiAgentPipelineService.cs:45-180 (future implementation)'
  },
  'voting': {
    explanation: 'Sistema di voting con 3 LLM diversi che generano risposte indipendenti, poi un aggregator sceglie il consenso.',
    technical: 'Parallel execution: Sonnet 4.5 + GPT-4o + DeepSeek generano simultaneamente. Aggregator (Sonnet) analizza le 3 risposte e sceglie quella con maggior consenso o merge insights.',
...
const STRATEGY_FLOWS: Record<RagStrategy, FlowBlock[]> = {
  FAST: [
    ...
    },
    {
      ...
      technicalDetails: 'QaResponseDto con: Answer, Sources[], Confidence, ValidationResults. Serializzato come JSON e restituito via API.',
  ...
  PRECISE: [
    ...
    },
    {
      ...
      technicalDetails: 'Multi-Agent Pipeline: Analyzer + Validator + Synthesizer. Reasoning sequenziale con handoff.',
    ...
    },
    {
      ...
      technicalDetails: 'Usa Haiku 4.5 per valutare relevance di ogni chunk (0-1 score). Scarta chunk con score < 0.7. Risparmia tokens nella generation.',
    ...
    },
    {
      ...
      technicalDetails: 'Include: Answer, Sources[], Confidence, MultiModelAgreement, CitationCoverage, HallucinationScore, AccuracyScore.',
      codeReference: 'AskQuestionQueryHandler.cs:180-200'
    }
  ],
