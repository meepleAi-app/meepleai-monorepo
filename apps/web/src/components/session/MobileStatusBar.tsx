  ...
  return (
    <div
      ...
      className={cn(
        ...
        'bg-[var(--glass-bg)] border-b border-[var(--glass-border)] text-sm text-[var(--text)]'
      )}
    >
      <div className="flex items-center gap-2 min-w-0">
        ...
        <span className="text-xs text-[var(--text-sec)] truncate">· {currentPlayer}</span>
      </div>

      <div className="flex items-center gap-3 shrink-0">
        <span className="text-xs text-[var(--text-sec)]">Turno {currentTurn}</span>
        {isLive && (
          <button
            onClick={togglePause}
